...
use reqwest::Client;
use system_core::traits::{Channel, ToolResult, ToolSpec};
...
use system_core::traits::{Memory, MemoryCategory, MemoryQuery, MemoryScope};

/// Tool for querying project health, bead counts, and worker states.
...
impl Tool for ProjectStatusTool {
    async fn execute(&self, args: serde_json::Value) -> Result<ToolResult> {
        ...
        for ds in &status.projects {
            ...
            output.push_str(&format!(
                ...
                ds.name, ds.open_quests, ds.ready_tasks,
    ...
    fn spec(&self) -> ToolSpec {
        ToolSpec {
            ...
            description: "Get project health, bead counts, and worker states. Optionally filter by project name.".to_string(),
...
/// Tool for assigning a task (bead) to a target project.
...
impl Tool for ProjectAssignTool {
    async fn execute(&self, args: serde_json::Value) -> Result<ToolResult> {
        ...
        match self.registry.assign(project, subject, description).await {
            Ok(bead) => Ok(ToolResult::success(format!(
                "Assigned {} [{}] {} to project '{}'",
                bead.id, bead.priority, bead.subject, project
            ))),
    ...
    fn spec(&self) -> ToolSpec {
        ToolSpec {
            ...
            description: "Assign a task to a specific project by creating a bead on it.".to_string(),
            input_schema: serde_json::json!({
                "type": "object",
                "properties": {
...
impl Tool for MailReadTool {
    async fn execute(&self, _args: serde_json::Value) -> Result<ToolResult> {
        let messages = self.dispatch_bus.read("familiar").await;
...
impl Tool for MailSendTool {
    async fn execute(&self, args: serde_json::Value) -> Result<ToolResult> {
        ...
        self.dispatch_bus.send(Dispatch::new_typed("familiar", to, kind)).await;
    ...
    fn spec(&self) -> ToolSpec {
        ToolSpec {
            ...
            description: "Send a mail message to another project or agent through the mail bus.".to_string(),
            input_schema: serde_json::json!({
                "type": "object",
                "properties": {
...
/// Tool for listing all unblocked beads across all projects.
...
impl Tool for AllReadyTool {
    async fn execute(&self, _args: serde_json::Value) -> Result<ToolResult> {
        let ready: Vec<(String, system_tasks::Task)> = self.registry.all_ready().await;
        ...
        let mut output = String::new();
        for (project_name, bead) in &ready {
            output.push_str(&format!(
                "[{}] {} [{}] {} — {}\n",
                project_name, bead.id, bead.priority, bead.subject,
    ...
    fn spec(&self) -> ToolSpec {
        ToolSpec {
            ...
            description: "List all unblocked beads across all projects that are ready for work.".to_string(),
            input_schema: serde_json::json!({
                "type": "object",
                "properties": {}
            }),
...
impl Tool for ChannelReplyTool {
    async fn execute(&self, args: serde_json::Value) -> Result<ToolResult> {
        ...
        let outgoing = system_core::traits::OutgoingMessage {
...
impl Tool for QuestCancelTool {
    async fn execute(&self, args: serde_json::Value) -> Result<ToolResult> {
        ...
        let reason = args
            .get("reason")
            .and_then(|v| v.as_str())
            .unwrap_or("Cancelled by focal agent");
...
impl Tool for QuestReprioritizeTool {
    async fn execute(&self, args: serde_json::Value) -> Result<ToolResult> {
        ...
        let priority = match priority_str.to_lowercase().as_str() {
            ...
            "normal" => system_tasks::Priority::Normal,
    ...
    fn name(&self) -> &str { "quest_reprioritize" }
}

/// Build orchestration tools for the focal agent.
///
/// NOTE: `channel_reply` is intentionally excluded. The focal agent's final text output

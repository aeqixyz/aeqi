...
use sigil_beads::{BeadId, BeadStore};
use std::collections::HashMap;
use std::path::Path;

/// A molecule is a workflow template: a sequence of steps with dependencies.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Molecule {
    pub molecule: MoleculeMeta,
    #[serde(default)]
...
impl Molecule {
    /// Load a molecule from a TOML file.
    pub fn load(path: &Path) -> Result<Self> {
        let content = std::fs::read_to_string(path)
            .with_context(|| format!("failed to read molecule: {}", path.display()))?;
        toml::from_str(&content)
            .with_context(|| format!("failed to parse molecule: {}", path.display()))
    }

    /// Pour (instantiate) a molecule: create a parent bead with child step beads.
    pub fn pour(
        &self,
        store: &mut BeadStore,
        prefix: &str,
        ...
        // Create parent bead.
        let parent_subject = self.interpolate(&self.molecule.description, vars);
        let parent = store.create(prefix, &parent_subject)?;

        // Create step beads as children.
        let mut step_bead_ids: HashMap<String, BeadId> = HashMap::new();
        ...
        for step in &self.steps {
            ...
            step_bead_ids.insert(step.id.clone(), child.id.clone());
        }

        // Wire up dependencies based on `needs`.
        for step in &self.steps {
            if let Some(step_bead_id) = step_bead_ids.get(&step.id) {
                for need in &step.needs {
                    if let Some(dep_id) = step_bead_ids.get(need) {
                        store.add_dependency(&step_bead_id.0, &dep_id.0)?;
                    }
                }
            }
        }
